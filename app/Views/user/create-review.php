...
<div class="container mx-auto px-4 py-8">
    ...
    <nav class="mb-6 text-sm" aria-label="Breadcrumb">
        <ol class="list-none p-0 inline-flex space-x-2">
            ...
            <li class="flex items-center">
                <span class="text-muted-foreground mx-2">/</span>
                <span class="text-foreground">Tulis Ulasan</span>
            </li>
        </ol>
    </nav>

    <h1 class="text-3xl font-bold text-foreground mb-6">Tulis Ulasan untuk: <?= esc($course['title']) ?></h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-background border rounded-lg p-6">
                ...
                <form action="<?= site_url('course/' . $course['id'] . '/reviews') ?>" method="post">
                    <?= csrf_field() ?>
                    <div class="mb-6">
                        ...
                        <div class="flex items-center space-x-1 rating-stars">
                            <?php for ($i = 5; $i >= 1; $i--): ?>
                                <input class="sr-only rating-input" type="radio" name="rating" id="rating-<?= $i ?>" value="<?= $i ?>" required <?= old('rating') == $i ? 'checked' : '' ?>>
                        ...
                        <input type="hidden" name="rating_value" id="rating_value_input" value="<?= old('rating') ?>">
                    </div>
                    
                    <div class="mb-6">
                        <label for="review" class="block text-sm font-medium text-foreground mb-2">Ulasan Anda</label>
                        <textarea class="w-full px-3 py-2 border border-muted rounded-md focus:outline-none focus:ring-2 focus:ring-primary min-h-[120px]" id="review" name="review" placeholder="Bagikan pengalaman Anda mengikuti kursus ini... (minimal 10 karakter)" required><?= old('review') ?></textarea>
            ...
            <div class="sticky top-8 space-y-6">
                ...
                <div class="bg-background border rounded-lg p-6">
                    ...
                    <ul class="space-y-1.5 text-xs text-muted-foreground list-disc list-inside">
                        <li>Bagikan pengalaman belajar Anda dengan jujur dan konstruktif.</li>
                        <li>Berikan detail spesifik tentang apa yang Anda pelajari dan bagaimana kursus ini membantu Anda.</li>
                        <li>Sebutkan aspek apa yang paling Anda sukai atau mungkin bisa ditingkatkan.</li>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeStars() {
        ...
        ratingLabels.forEach((label, index) => {
            const starValue = 5 - index; // Stars are in reverse order in HTML (5 to 1)
    ...
    function updateStars(currentRating) {
        ratingLabels.forEach((lbl, idx) => {
            const val = 5 - idx; // Stars are in reverse order
    ...
    initializeStars(); // Call on page load
});
</script>
<?= $this->endSection() ?>
